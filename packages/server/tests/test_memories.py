...
async def test_memories_api(mock_mem0_client):
    ...
    mock_mem0_client.get_all.return_value = {
        "count": 1,
        "results": [
            {"id": "mem-1", "memory": "test memory text", "user_id": session_id}
        ],
    }
    ...
    async with AsyncClient(
        ...
        mock_mem0_client.get_all.assert_called_once_with(
    ...
    async with AsyncClient(
        ...
        res = await ac.post(
            f"/api/sessions/{session_id}/memories/search", json={"query": "programming"}
        )
        assert res.status_code == 200
        data = res.json()
